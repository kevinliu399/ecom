...
func (h *Handler) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.WriteError(w, http.StatusInternalServerError, fmt.Errorf("not found, invalid email %s or password", user.Email))
		return
	}

	if !auth.ComparePasswords(u.Password, []byte(user.Password)) {
		utils.WriteError(w, http.StatusUnauthorized, fmt.Errorf("not found, invalid email %s or password", user.Email))
		return
	}
